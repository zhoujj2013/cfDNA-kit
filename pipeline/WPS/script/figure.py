...
gene_intensity_ratio_path = args.gene_intensity_path
patient_cell_corelation_ratio_path = args.patient_cell_corelation_path
single_cell_expression = args.single_cell_expression
sample_file = args.sample_file
save_path = args.save_path

ensg_gene_file = '/mnt/dfc_data2/project/linyusen/project/cfDNA_kit/pipeline/WPS/database/ensg_offic_gene'
# gene_intensity_ratio_path = '/mnt/dfc_data2/project/linyusen/database/46_cfdna/newdata/compare_old/summary/gene_intensity_198.pkl'
# patient_cell_corelation_ratio_path = '/mnt/dfc_data2/project/linyusen/database/46_cfdna/newdata/compare_old/summary/patient_cell_corelation_198.pkl'
# single_cell_expression = '/mnt/dfc_data2/project/linyusen/database/46_cfdna/newdata/plasma_100/single_cell/single_cell.expression.csv'
# sample_file = '/mnt/dfc_data2/project/linyusen/database/46_cfdna/newdata/compare_old/sample.lst'
# save_path = '/mnt/dfc_data2/project/linyusen/database/46_cfdna/newdata/compare_old/figure_198'
#%%
...
for type in sample_list.keys():
    ...
    for tissue in x_ticks:
        data[type][tissue] = tissue_rank_dict[type][tissue]
...
for patient in patient_cell_corelation:
    sorted_items = sorted(
        patient_cell_corelation[patient].items(),
        key=lambda x: x[1],
        reverse=True
    )
    rank_dict = {key: rank + 1 for rank, (key, _) in enumerate(sorted_items)}
    patient_rank_dict[patient] = rank_dict
import pandas as pd
import os

df_rank = pd.DataFrame(patient_rank_dict)
df_rank.to_csv(os.path.join(save_path, 'rank_dict.csv'),)
#%%
